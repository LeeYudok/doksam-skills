...
| --- | --- | --- |
| **이슈 도중** — 작업 이슈가 아직 열려 있다(컨텍스트 40~50% 등) | 전용 핸드오프 이슈 + `HANDOFF.md`(포인터, 아래 "두 갈래") | 핸드오프 이슈 URL |
...
| | 하는 일 | `HANDOFF.md` | 다음 세션이 받는 것 |
| --- | --- | --- | --- |
| **인프라 있음** | 본문을 스크래치 파일에 쓰고 **그 내용으로 이슈/티켓/스레드를 만든다** | **포인터만**(이슈 URL·작성 시각·요약·목록 링크) | **전체 URL** |
| **인프라 없음** | `HANDOFF.md` 에 전체 내용을 쓴다 | **본문 그대로 = 원본** | **파일 경로** |

이슈 도중에 끊을 때 본문은 **항상** 만든다. 인프라가 있으면 그 본문은 이슈에
들어가고 `HANDOFF.md` 는 위치만 가리킨다 — 레포에 재개 상태를 평문으로 남기지
않고, 다음 세션은 파일 한 줄만 읽어도 어디로 가야 하는지 안다. 인프라가 없으면
...
| 호출 | 하는 일 |
| --- | --- |
| `/handoff` (인자 없음) | **작성** — 이슈 도중이면 인프라 있을 때 이슈를 만들고 `HANDOFF.md` 엔 포인터만, 인프라 없으면 `HANDOFF.md` 에 전체 내용; 이슈 경계면 다음 작업 이슈 코멘트 → URL 또는 파일 경로 출력 |
...
### 1-2. 본문을 스크래치 파일에 쓴다 (템플릿은 아래)

레포의 `HANDOFF.md` 에 바로 쓰지 않는다 — 인프라가 있으면 이 본문은 이슈로 가고
파일에는 포인터만 남기 때문이다. 세션 스크래치 디렉터리(또는 `/tmp`) 에 임시
파일로 아래 "본문 템플릿" 전 항목을 채워 쓴다.

인프라가 전혀 없을 때만(0번 판별 결과 "아무것도 없음") 이 본문을 그대로 레포
루트 `HANDOFF.md` 에 쓴다 — 이미 있으면 **덮어쓴다**. 핸드오프는 항상 "지금
상태" 한 장이고, 과거 이력은 트래커와 그 레포의 작업 로그 파일
...
python3 -c "
import json;json.dump({'title':'핸드오프: <요약>','body':open('<본문파일>.md').read()}, open('payload.json','w'))"
...
### 1-4. `HANDOFF.md` 에 포인터를 쓰고 커밋한다

인프라가 있으면 레포 루트 `HANDOFF.md` 를 아래 "포인터 템플릿"으로 (덮어)쓴다 —
...
2. **템플릿 전 항목을 채운 코멘트 본문을 파일로 쓴다**(아래 "본문 템플릿" 그대로,
...
`# 종료 — <날짜>` 와 다음 작업 이슈 URL 을 적어 낡은 포인터를 최신으로 오해하지
않게 한다.
...
인프라가 있으면 `HANDOFF.md` 는 **포인터**다 — 먼저 그 파일을 읽고 적힌 이슈
URL 을 연다. 파일 자체를 재개 상태로 쓰지 않는다. 인프라가 없으면 파일 자체가
본문 전체다.

```bash
cat HANDOFF.md   # 포인터: 이슈 URL 을 얻는다 (인프라 없으면 이 파일이 본문 전체)
...
### 본문 — 스크래치 파일 → 이슈/코멘트 본문, 인프라 없으면 `HANDOFF.md`
...
세션 인계 본문은 <트래커> 이슈에 있다. 이 파일은 그 위치만 가리킨다.
...
- **인프라 있으면 원본은 이슈, `HANDOFF.md` 는 포인터. 인프라 없으면 원본은 파일**.
