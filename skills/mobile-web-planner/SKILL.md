...
**`09.x` 슬라이드에는 `ppt-meta-bar` 로 화면 위치를 적는다.** 진입점부터 그 화면까지의 경로를 `>` 로 잇는다 — 예: `홈 > 게시판 > 글 상세`. `04 IA` 의 연결 관계에서 그대로 끌어온다. 이 줄이 있으면 IA 슬라이드를 넘겨보지 않아도 화면이 앱 어디에 있는지 알 수 있다. 화면 상세(`09.x`) 외 슬라이드에는 넣지 않는다 — 화면이 아니므로 위치가 없다.
...
| `ppt-meta-bar` | 상단 바 아래 메타 줄. **화면 상세(`09.x`)에만** 둔다 |
...
파일을 저장하기 전에 완성된 마크업을 훑으며 아래 열세 가지를 센다. 어긋나는 항목이 있으면 저장 전에 고친다. 템플릿의 CSS 를 그대로 옮기지 않고 다시 썼더라도 이 점검은 그대로 수행한다.
...
7. **화면 위치** — `09.x` 마다 `ppt-meta-bar` 가 있고 값이 `04 IA` 의 경로와 맞는가. 화면 상세 외 슬라이드에는 없어야 한다.
