...
파일을 저장하기 전에 완성된 마크업을 훑으며 아래 열여섯 가지를 센다. 어긋나는 항목이 있으면 저장 전에 고친다. 템플릿의 CSS 를 그대로 옮기지 않고 다시 썼더라도 이 점검은 그대로 수행한다.
...
16. **이벤트 커버리지** — 목업의 터치 가능 요소(버튼·탭·행·칩·토글·FAB·입력)마다 배지가 있는가. 인터랙티브 설명 항목마다 이벤트 라벨(`탭:` `스와이프:` `롱프레스:` `입력:`) 줄이 있는가. storyboard 의 모든 이벤트가 Business Rules `### 인터랙션` 표에 대응 행을 갖는가.
